...
spread_row <- function(row_i){
	...
	spread_row <- tidyr::separate(row_i,col=last(names(row_i)),sep=":", into=unlist(str_split(row_i[1,"FORMAT"],":")), remove=TRUE) %>%
	dplyr::select(-FORMAT)  %>%  tidyr::separate(.,col="INFO",sep=";", into=info_tag_names, remove=TRUE) 
